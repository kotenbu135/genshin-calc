...
- `talent_buffs/`: 元素別天賦バフファイル（pyro/hydro/electro/cryo/dendro/anemo/geo — 各ファイルに TalentBuffDef 定義）
- `team_builder.rs`: TeamMemberBuilder（キャラ+武器+聖遺物→TeamMember構築）
- `moonsign_chars.rs`: 月兆キャラデータ（月光の祝福パッシブ + タレント強化）
- `characters/`: 元素別ディレクトリ構造
  - `characters/{element}/mod.rs`: 元素ごとの CHARACTERS スライス（例: `pyro/mod.rs` で `CHARACTERS: &[CharacterData]`）
  - `characters/{element}/{char}.rs`: キャラごとのファイル（例: `pyro/durin.rs` で `pub const DURIN: CharacterData`）
  - `characters/mod.rs`: 全キャラ統合ポイント（各元素モジュール再エクスポート、`all_characters()` 関数）
